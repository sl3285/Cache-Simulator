...
double BaseCache::getReadHitRate(){
  return 100*numReadHits/(numReadHits+numReadMisses);
  // return 100*numReadHits/numReads;  
}
double BaseCache::getReadMissRate(){
  return 100*numReadMisses/(numReadHits+numReadMisses);
  // return 100*numReadMisses/numReads;
}
double BaseCache::getWriteHitRate(){
  return 100*numWriteHits/(numWriteHits+numWriteMisses);
  // return 100*numWriteHits/numWrites;
}
double BaseCache::getWriteMissRate(){
  return 100*numWriteMisses/(numWriteHits+numWriteMisses);
  // return 100*numWriteMisses/numWrites;
}
double BaseCache::getOverallHitRate(){
  return 100*(numReadHits+numWriteHits)/(numReadHits+numWriteHits+numReadMisses+numWriteMisses);
  // return 100*(numReadHits+numWriteHits)/(numReads+numWrites);
}
double BaseCache::getOverallMissRate(){
  return 100*(numReadMisses+numWriteMisses)/(numReadMisses+numWriteMisses+numReadHits+numWriteHits);
...
bool BaseCache::write(uint32_t addr, uint32_t data, uint32_t full) {
 numWrites++;
 uint32_t tagAddr=addr>>(ADDR_BITS-TagBits);
 uint32_t indexAddr=(addr<<TagBits)>>(TagBits+OffsetBits);
 if (IndexBits == 0)
...
void BaseCache::Address(uint32_t offsetAddr, uint32_t indexAddr, uint32_t tagAddr)
{
  uint32_t a,b,c;
  uint32_t d,e,f;
  d=OffsetBits;
  e=IndexBits+OffsetBits;
  f=IndexBits+OffsetBits+TagBits;
  a=pow(10,d);
  b=pow(10,e);
  c=pow(10,f);
  offsetAddr=ADDR_BITS % a;
  indexAddr=(ADDR_BITS % b - offsetAddr)/a;
  tagAddr=(ADDR_BITS % c - indexAddr - offsetAddr)/b;
}
